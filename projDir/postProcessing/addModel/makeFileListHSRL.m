function [fileList] = makeFileListHSRL(indir,startTime,endTime,fileFormat)
...
for jj=1:size(allDays,2)
    yearFile=num2str(year(allDays(jj)));
    if month(allDays(jj))<10
        monthFile=['0',num2str(month(allDays(jj)))];
    else
        monthFile=num2str(month(allDays(jj)));
    end
    if day(allDays(jj))<10
        dayFile=['0',num2str(day(allDays(jj)))];
    else
        dayFile=num2str(day(allDays(jj)));
    end
    subFileList=dir([indir,yearFile,'/',monthFile,'/',dayFile,'/raw/*_data_fl1.nc']);
